...
final class MarkerIconFactory {
    ...
    func image(for status: ReportStatus, size: MarkerSize, selected: Bool, scale: CGFloat, zoomMultiplier: CGFloat = 1.0, interfaceStyle: UIUserInterfaceStyle = .light) -> UIImage {
        let quantizedMultiplier = round(zoomMultiplier / 0.05) * 0.05
        let styleKey = interfaceStyle == .dark ? "dark" : "light"
        let cacheKey = "pin-\(status)-\(size)-\(selected)-@\(Int(scale))-\(String(format: "%.2f", quantizedMultiplier))-\(styleKey)" as NSString
        
        if let cached = cache.object(forKey: cacheKey) {
            return cached
        }
        
        let image = drawTeardropMarker(status: status, size: size, scale: scale, zoomMultiplier: zoomMultiplier, interfaceStyle: interfaceStyle)
        cache.setObject(image, forKey: cacheKey)
        return image
    }
    
    private func drawTeardropMarker(status: ReportStatus, size: MarkerSize, scale: CGFloat, zoomMultiplier: CGFloat, interfaceStyle: UIUserInterfaceStyle) -> UIImage {
